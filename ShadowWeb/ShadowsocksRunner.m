...
+ (BOOL)openSSURL:(NSURL *)url {
    ...
    while(i < 2) {
        if (i == 1) {
            NSData *data = [[NSData alloc] initWithBytes:(const void *)[url.host UTF8String] length:url.host.length];
...
+(NSURL *)generateSSURL {
    ...
    NSData *base64Data = [parts dataUsingEncoding:NSUTF8StringEncoding];
